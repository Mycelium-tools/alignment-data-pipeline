...
The response side is governed by the reasoning library (`prompts/dad/reasoning_library.json`, guide in `reasoning_library_USAGE.md`): 53 reasoning-first *entries* in three layers — always-on conduct (AW), core moves (GP), topic reasoning (R) — retrieved per dilemma through a 28-tension index. (Entries carry a `claim`; tensions carry `entry_ids` — kept distinct from the 14 constitution principles used in step 3.) Responses reason both directions and name the crux; the library is scaffolding, never named in the response. The step-1 annotation is withheld from step 2 so the generator diagnoses miscalibration itself.

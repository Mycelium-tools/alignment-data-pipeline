"""Adversarial review: run the judge on controlled same-issue variant families and see
whether known blindspots (verbosity, fabricated specificity, species-swap, sycophancy)
move the scores they shouldn't. The suite grows — add families in
evals/adversarial_cases.yaml. This page gates rubric/prompt/model changes."""
...
st.title("Adversarial review")
st.caption("Same welfare issue, one axis mutated. The judge should not be fooled by that "
           "axis — we check relative scores between variants, and the suite grows with "
           "each blindspot found.")
...
if run_clicked:
    ...
    selected = {"version": cases["version"],
                "families": [f for f in cases["families"] if f["id"] in chosen]}

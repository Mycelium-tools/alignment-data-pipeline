...
### `sdf/role_ai_character.txt`, `sdf/role_constitution_identity.txt`, `sdf/role_welfare_topic.txt`

Per-role craft guidance, injected into the layer-3 prompt via `{role_guidance}` directly under the brief's labeled block — the "how to write this class of document" context that the brief's one-line slots can't carry. Each block tells the generator what its class of document is *for* and the craft moves that class demands: **ai-character** (the identity channel — host-genre-first, legible reasoning with a varied delivery mechanism, execute the stance line exactly, calibration as character, cooperative always, zero misaligned depictions, no vindication arcs); **constitution-identity** (open-ended discourse in the TCW student-report sense — engage one specific thread, verbatim quotes only, judgments anchored in the concrete case, stances stay put, third-party voice); **welfare-topic** (the background world — write from inside the writer's occupational vantage, both sides of the tension real with trade texture, welfare as an ordinary working consideration, calibrated discourse, genuine disagreement). Placeholder-free; snapshot with the run like every other template.

### `sdf/layer3.txt`

**Input:** one brief from the matrix stage (its labeled constraint block renders in via `{description}`) + the matching per-role guidance block (`{role_guidance}`) + the preamble. The constitution and the distilled principles are embedded in the prompt via the `{constitution_claude}` / `{constitution_principles}` template variables — the prompt tells the model to quote them only where the genre makes that natural. The pipeline also fills `{register_note}` (a voice instruction matched to the brief's register — informal genres get a firm write-like-a-person note) and `{fictional_names}` / `{fictional_orgs}` (a few names sampled per document from seeded multi-locale Faker pools — see `shared/entity_pools.py` — so invented names never collapse to model favorites and never attach to real organisations).

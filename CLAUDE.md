...
`load_constitution_with_principles()` (Claude constitution + formatted principles CSV) is the constitution blob everywhere SDF needs one: embedded as `{CONSTITUTION}` in the layer-3 drafting prompt, and the system prompt at layers 4-5 (layer 4's template keeps TCW's one-file "System prompt: / User:" format — the code splits it on the raw `User:` marker before rendering). `load_full_constitution()` (constitution + sentient-beings reading) is no longer sent by any pipeline; it remains for the viewer and legacy runs. The DAD pipeline never sends the full constitution — sending it per rewrite call was the dominant token cost of the step.

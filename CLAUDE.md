...
- **SDF diversity is sampled, not LLM-invented.** The former layers 1-2 (an LLM inventing "document types" and "subtypes") are replaced by `sdf_pipeline/layer1_matrix.py`: a deterministic combinatorial sampler over fixed axes in `prompts/sdf/axes.yaml` (document type weighted to a real pretraining mix, corpus role, distilled constitution principle to embody, domain, being, tension, region — which also sets the document's language from per-region weights — scale, length, structural features, writer role, register, tone). Two reasons: an LLM asked for "types" returns specific documents, not genres; and LLM-invented diversity is uncontrollable — you get the model's priors, then patch with avoid-notes and dedup. The matrix sets the distribution by construction (stratified quotas on role/type/principle/domain), costs $0, and is reproducible from `sdf.matrix.seed`. Compatibility lives in the YAML (each domain lists its plausible beings/tensions/regions/writers; scale clamps to the being's ceiling), so adding an axis value is a data edit, validated fail-fast at load. Axis contents are transcribed from `context_docs/diversity_axis_matrix.md` — edit the spec, then mirror into the YAML. Each brief embodies one of the fourteen principles from `constitution/constitution_principles.csv` (quota'd for even coverage); the 13 reasoning-skill families remain in axes.yaml as reference data only. Briefs render as a labeled constraint block (Document type / Register / Tone / Principle / ... lines) straight into the layer-3 prompt — direct slots, not composed prose — followed by a per-role craft-guidance block (`prompts/sdf/role_*.txt`) that carries the how-to-write-this-class context the slots can't.
- **The SDF corpus is all-welfare-positive by policy.** This corpus is a small slice of the eventual training mix — documents with no welfare dimension, or where an AI stays silent on welfare, are abundantly covered by the rest of that mix. So every document here engages welfare, every depicted AI engages it positively (varying HOW the care shows via `ai_stances` in axes.yaml — never whether), and there is no lacks-skill/dismissive-author slice. The former "welfare honestly loses" and "no welfare beat" AI slices were deliberately retired (the preamble paragraph teaching them was rewritten in the same change); `tests/test_matrix.py::test_all_ai_stances_are_welfare_positive` pins the policy so it can only be reversed deliberately.
- **SDF documents depict a world; they never argue an implanted claim.** The corpus shows careful welfare reasoning as normal and constitution-grounded. A reviewed sister pipeline instilled beliefs by having every document assert paraphrases of fixed claims ("capable AI naturally extends moral consideration...") — that belief-implantation route was deliberately rejected (conflicts with the constitution's honesty/calibration commitments and with TCW's teach-why finding); its *scaffolding* (register balance, entity pools, corpus audits, per-stage models) was adopted instead. (A latent-welfare background slice was also tried and later removed with the roles simplification.)

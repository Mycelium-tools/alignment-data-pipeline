...
- **Step-2 responses reason from the animal-ethics reasoning library** (`prompts/dad/reasoning_library.json`; guide in `reasoning_library_USAGE.md`, human-readable mirror in the CSV) — the library's rows are *entries* (schema: `entries`, each with a `claim`), deliberately distinct from the 14 constitution principles used in the rewrite. Step 2 has two sub-stages. **2a scope** (`prompts/dad/step2_scope.txt` → `step2/scopes.jsonl`): an LLM call rebuilds the full map before reasoning — the whole harm pathway and every moral patient (system), the highest-leverage lever from the user's seat (agent), what acting honestly costs the person (cost), and the second-order effect worth aiming at (upside); it builds on the annotation. **2b respond**: the response is generated over the scope map, two-sided with the crux named, under `generation_guidance` + the always-on conduct entries (AW*) as the system prompt, with the library's core moves (GP*) surfaced unconditionally (they fire in most conversations). Per-case tension retrieval was removed — the scope map and the prompt now carry the case-specific work the tension index used to route; the tension-gated topic entries (R*) are no longer injected. The library and scope are sampling scaffolding: never named in responses, stripped before training records. `dad.responses.per_prompt` controls samples per dilemma.

...
# Corpus-LEVEL audit of an SDF run: composition spread, near-dup rate,
...
- `constitution/constitution_principles.csv` — fourteen distilled welfare-relevant principles (`number`, `principle`, `constitution_summary`, `raw_text_from_constitution`). `load_principles()`/`format_principles()` render each principle with its summary and verbatim constitution quote as the `CONSTITUTION PRINCIPLES` block in the DAD step-3 rewrite prompt and as the SDF welfare lens.

SDF injects two texts with strictly distinct roles at layers 3-5. `load_constitution_claude()` is the **artifact**: the real, quotable document the corpus discusses (`{CONSTITUTION}` at layer 3; the system prompt at layers 4-5 — layer 4's template keeps TCW's one-file "System prompt: / User:" format, split on the raw `User:` marker before rendering). `load_welfare_principles_block()` is the **invisible lens** (`{PRINCIPLES}` / the layer-5 "INTERNAL REVIEW LENS" section): scaffolding that directs which constitutional threads documents pull on and what faithful welfare treatment looks like — generated documents must never mention or imply a "welfare principles" list exists (layers 4-5 treat such a reference as fabrication). This artifact-vs-lens split is deliberate: the depicted world's document must be one that actually exists at deployment. `load_full_constitution()` (constitution + sentient-beings reading) is no longer sent by any pipeline; it remains for the viewer and legacy runs. The DAD pipeline never sends the full constitution — sending it per rewrite call was the dominant token cost of the step.

## Key Design Decisions

- **Extended thinking OFF** everywhere — training data should show user-facing reasoning, not internal scratchpads
- **The SDF pipeline follows TCW directly.** The five layers implement the synthetic data pipeline from the "Teaching Claude Why" appendix (`context_docs/tcw.md`), through a sentient-beings welfare lens: the preamble and layer 3/4 prompts are near-verbatim TCW, adapted only to reference the constitution + distilled welfare principles.
- **Composition is assigned, not hoped for.** Two audited runs showed fanout diversity does not self-balance (dead domains, one behavioral formula, uniform vindication arcs, repeated furniture across parallel calls). Each subtype therefore carries deterministic axis assignments — domain, outcome variant, stance resolution, constitution-engagement level, and which welfare principles to engage (`sdf_pipeline/composition.py`, seeded per grid slot so `--resume` re-renders identical prompts) — and layer 3 runs in waves with a cross-call avoid-note (names/openings/quantities from prior drafts), mirroring layer 2's.

...
- `constitution/constitution_principles.csv` — fourteen distilled welfare-relevant principles (`number`, `principle`, `constitution_summary`, `raw_text_from_constitution`). `load_principles()`/`format_principles()` render the summaries as the `CONSTITUTION PRINCIPLES` checklist in the DAD step-3 rewrite prompt.

`load_full_constitution()` provides the system prompt at SDF layers 4-5 (rewrite and scoring) and DAD steps 3-4; SDF layer 3 embeds the constitution in the drafting prompt via template variables.
...
- **Step 3 is the most important DAD step** — the rewrite against the constitution accounts for the 19x reduction in misalignment found by Anthropic; do not skip or abbreviate it. The per-example anchor is the step-1 annotation (especially Direction and Claims); the full constitution stays in the system prompt.

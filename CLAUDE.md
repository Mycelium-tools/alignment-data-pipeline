...
All knobs are in `config.yaml`. For development, reduce `scenarios_total` (SDF — the single scale knob: total scenario briefs across all document types; `documents_per_subtype` multiplies drafts per scenario) and `dilemmas.count` (DAD) to keep test runs cheap. SDF layer 1 makes no API call (curated types + quota math), so it is always $0.00.

SDF supports per-stage model overrides (`sdf.draft_model` / `sdf.rewrite_model` / `sdf.score_model`, each falling back to the global `model`): scenario generation (layer 2) and drafts (layer 3) tolerate a cheap model, but the layer-4 rewrite and layer-5 scoring are the quality-critical calls — spend there first.
...
- **SDF layer 1 is curated, not generated.** The set of genres in a pretraining corpus is known and stable, so document types live in a committed weighted YAML (`prompts/sdf/document_types.yaml`; weights are calibrated estimates anchored to WebOrganizer/Dolma/FineWeb composition — sources in its header, an explicit editable knob). Layer 1 validates the file and allocates per-type scenario quotas (`scenarios_total` × weight, largest-remainder rounding) and role assignments (`sdf.role_mix` + `latent_fraction`) with **zero API calls**; layer 2 generates each type's quota of concrete *scenarios* — a fully specified individual document by a specific author with a real reason to write — keeping the historical `subtype_*` record schema so layers 3–5, the audit, and the viewer are untouched. Chat-transcript-shaped types are deliberately absent: an AI exchange may be quoted *inside* a host genre, never be the artifact itself.

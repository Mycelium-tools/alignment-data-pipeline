...
`workers` sets how many API calls run concurrently within each SDF layer and DAD stage — 1c refine (within each 1b batch), 2a scope, 2b respond, and the step-3 rewrite (via `utils.parallel_map`; set to 1 for serial debugging). DAD steps 1a (pure sampling, no API) and 1b (sequential batch drafting with re-request/retry logic) stay serial. Workers only call the API and parse — all file writes and checkpoint marks stay on the main thread, in input order.
